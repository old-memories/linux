...
static int ublk_ch_release(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&ub->mutex);
	/* When we are here, we know that:
	 * (1) ublk kernel resources have not be freed by ublk_cdev_rel()
	 *     because we have not called ublk_put_device(ub) yet. So we
	 *     are safe to reference ub.
	 * (2) monitor_work has been canceled so ub's state is DEAD or
	 *     QUIESCED. Also all ioucmds are completed and no inflight rq
	 *     exists. So we are safe to reinit queues and reset ub->mm.
	 */
